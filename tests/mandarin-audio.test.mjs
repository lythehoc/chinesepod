...
const manifest = JSON.parse(readFileSync(new URL("../app/data/mandarin-audio.json", import.meta.url), "utf8"));
const compiled = ts.transpileModule(readFileSync(new URL("../app/lib/use-mandarin-audio.ts", import.meta.url), "utf8"), {
...
function harness(context, overrides = {}) {
  ...
  let options = { lessonId: 1, lines: [{ hanzi: "小幸运" }, { hanzi: "青春" }], rate: 1, loop: false, autoplayNext: false, onNext: () => nextCalls++, onPositionChange: (index) => positions.push(index), ...overrides };
  ...
  vm.runInNewContext(compiled, {
    ...
    require: (name) => {
      ...
      if (name === "../data/mandarin-audio.json") return { default: manifest };
  ...
  function render() {
    ...
    do {
      ...
      output = mod.exports.useMandarinAudio(options);
...
test("Mandarin playback uses bundled audio and waits for a real playing event", (t) => {
  const h = harness(t);
  h.hook.play();
  assert.equal(h.media.src, `/project${manifest["小幸运"]}`);
  assert.equal(h.hook.isPlaying, false);
  h.media.onplaying();
  assert.equal(h.hook.isPlaying, true);
  h.media.onended();
  assert.equal(h.hook.activeLine, 1);
  assert.equal(h.media.src, `/project${manifest["青春"]}`);
...
test("seeking clamps and saves position while vocabulary plays only once", (t) => {
  ...
  h.hook.speakText("小幸运"); h.media.onplaying(); h.media.onended();
  assert.equal(h.hook.activeLine, 1);
