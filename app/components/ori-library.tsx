...
export default function OriLibrary({ theme, onToggleTheme }: { theme: "light" | "dark"; onToggleTheme: () => void }) {
  ...
  return <main className="ori-page">
    <header className="ori-intro"><div><h1>Ori Princess <span lang="zh-Hans">甜心格格</span></h1><p>Learn Mandarin from every episode with a focused phrase, vocabulary, and three listening passes.</p></div><button className="ori-theme-toggle" onClick={onToggleTheme} aria-label={`Switch theme to ${theme === "light" ? "dark" : "light"}`}><UiIcon name={theme === "light" ? "moon" : "sun"} /><span>{theme === "light" ? "Dark" : "Light"}</span></button></header>
    <div className="ori-columns"><section className="ori-viewer" aria-label="Ori Princess video player">
      ...
      <p className="ori-pin-note"><span aria-hidden="true">●</span> Video stays in view while you scroll the lesson.</p>
