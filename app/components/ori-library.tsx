...
export default function OriLibrary() {
  ...
  return <main className="ori-page">
    ...
    <div className="ori-columns"><section className="ori-feature">
      <label className="ori-episode-picker">Episode<select aria-label="Choose Ori episode" value={episode.videoId} onChange={(event) => select(event.target.value)}>{episodes.map((item) => <option key={item.videoId} value={item.videoId}>Season 1 · Episode {item.id}</option>)}</select></label>
