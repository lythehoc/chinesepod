...
export default function OriLibrary({ theme, onToggleTheme }: { theme: "light" | "dark"; onToggleTheme: () => void }) {
  ...
  const learning = learningData.find((item) => item.videoId === episode.videoId) ?? learningData[0];
  useEffect(() => {
  ...
  return <main className="ori-page">
    <header className="ori-intro"><div><h1>Ori Princess <span lang="zh-Hans">甜心格格</span></h1></div><button className="ori-theme-toggle" onClick={onToggleTheme} aria-label={`Switch theme to ${theme === "light" ? "dark" : "light"}`}><UiIcon name={theme === "light" ? "moon" : "sun"} /><span>{theme === "light" ? "Dark" : "Light"}</span></button></header>
    <div className="ori-columns"><section className="ori-viewer" aria-label="Ori Princess video player">
      <label className="ori-episode-picker"><span>Episode</span><select aria-label="Choose Ori episode" value={episode.videoId} onChange={(event) => select(event.target.value)}>{episodes.map((item) => <option key={item.videoId} value={item.videoId}>{item.id}. {item.lessonTitle}</option>)}</select></label>
    ...
    </section><div className="ori-study-column">
      <section className="ori-language transcript-card" aria-labelledby="ori-language-title"><div className="card-heading"><div><span className="section-kicker">READ ALONG</span><h3>Transcript &amp; vocabulary</h3></div></div><div className="transcript-content"><div className="ori-phrase"><h2 id="ori-language-title" lang="zh-Hans">{episode.lessonTitle}</h2><p className="pinyin" lang="zh-Latn-pinyin">{episode.pinyin}</p><p className="translation">{episode.english}</p></div><div className="dialogue-block ori-transcript">{learning.transcript.map((line, lineIndex) => <article className="line" key={`${line.time}-${lineIndex}`}><span className="speaker">{clock(line.time)}</span><div className="text"><p className="hanzi" lang="zh-Hans">{line.hanzi}</p><p className="pinyin" lang="zh-Latn-pinyin">{line.pinyin}</p><p className="translation">{line.english}</p></div></article>)}</div><section className="public-vocabulary"><h2>Key Vocabulary</h2><div className="vocab-block">{learning.keyVocabulary.map((word) => <article className="vocab-item" key={word.hanzi}><div><div className="word" lang="zh-Hans">{word.hanzi}</div><div className="type" lang="zh-Latn-pinyin">{word.pinyin}</div><div className="definition">{word.english}</div></div></article>)}</div></section><section className="public-vocabulary"><h2>Supplementary Vocabulary</h2><div className="vocab-block">{learning.supplementaryVocabulary.map((word) => <article className="vocab-item" key={word.hanzi}><div><div className="word" lang="zh-Hans">{word.hanzi}</div><div className="type" lang="zh-Latn-pinyin">{word.pinyin}</div><div className="definition">{word.english}</div></div></article>)}</div></section></div></section>
